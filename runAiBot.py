...
def apply_filters() -> None:
    ...
    try:
        ...
        # Scope filter clicks to the filter panel so we don't click same-named elements elsewhere on the page
        filter_panel = None
        try:
            filter_panel = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, '[role="dialog"]')))
        except Exception:
            try:
                show_btn = wait.until(EC.presence_of_element_located((By.XPATH, '//button[contains(translate(@aria-label, "ABCDEFGHIJKLMNOPQRSTUVWXYZ", "abcdefghijklmnopqrstuvwxyz"), "apply current filters")]')))
                filter_panel = show_btn.find_element(By.XPATH, './ancestor::*[@role="dialog" or contains(@class, "modal") or contains(@class, "drawer")][1]')
            except Exception:
                pass

        # Scroll filter panel so top sections (Sort by, Date posted) are in view
        if filter_panel is not None:
            try:
                scroll_to_view(driver, filter_panel)
                driver.execute_script("arguments[0].scrollTop = 0;", filter_panel)
                buffer(0.3)
            except Exception:
                pass

        # Sort by (single select)
        wait_span_click(driver, sort_by, scope=filter_panel)
        buffer(recommended_wait)

        # Date posted: expand section then click option (time filter)
        if date_posted:
            expand_filter_section(driver, "Date posted", scope=filter_panel)
            buffer(0.5)
            if not click_filter_option(driver, date_posted, scope=filter_panel):
                wait_span_click(driver, date_posted, scope=filter_panel)
        buffer(recommended_wait)

        # Experience level: expand section then click each option
        if experience_level:
            expand_filter_section(driver, "Experience level", scope=filter_panel)
            buffer(0.5)
            multi_sel_filter_options(driver, experience_level, actions, scope=filter_panel)
        multi_sel_noWait(driver, companies, actions, scope=filter_panel)
        if experience_level or companies: buffer(recommended_wait)

        # Job type: expand section then click each option
        if job_type:
            expand_filter_section(driver, "Job type", scope=filter_panel)
            buffer(0.5)
            multi_sel_filter_options(driver, job_type, actions=None, scope=filter_panel)
        multi_sel_noWait(driver, on_site, scope=filter_panel)
        if job_type or on_site: buffer(recommended_wait)

        if easy_apply_only: boolean_button_click(driver, actions, "Easy Apply", scope=filter_panel)

        multi_sel_noWait(driver, location, scope=filter_panel)
        multi_sel_noWait(driver, industry, scope=filter_panel)
        if location or industry: buffer(recommended_wait)

        multi_sel_noWait(driver, job_function, scope=filter_panel)
        multi_sel_noWait(driver, job_titles, scope=filter_panel)
        if job_function or job_titles: buffer(recommended_wait)

        if under_10_applicants: boolean_button_click(driver, actions, "Under 10 applicants", scope=filter_panel)
        if in_your_network: boolean_button_click(driver, actions, "In your network", scope=filter_panel)
        if fair_chance_employer: boolean_button_click(driver, actions, "Fair Chance Employer", scope=filter_panel)

        wait_span_click(driver, salary, scope=filter_panel)
        buffer(recommended_wait)

        multi_sel_noWait(driver, benefits, scope=filter_panel)
        multi_sel_noWait(driver, commitments, scope=filter_panel)
        if benefits or commitments: buffer(recommended_wait)

        # Show results: try multiple selectors (LinkedIn may change aria-label)
        ...
        if show_results_button is None:
            raise Exception("Could not find 'Show results' / 'Apply' button in filters panel")
...
def get_job_description(
    ...
    try:
        ...
        if not skip and security_clearance == False:
            clearance_terms = ('polygraph', 'clearance', 'secret', 'top secret', 'ts/sci', 'ts sci')
            if any(term in jobDescriptionLow for term in clearance_terms):
                skipMessage = f'\n{jobDescription}\n\nJob requires or mentions security clearance. Skipping this job!\n'
...
def answer_questions(modal: WebElement, questions_list: set, work_location: str, job_description: str | None = None ) -> set:
    ...
    for Question in all_questions:
        ...
        if text_area:
            do_actions = False  # textarea (cover/summary) doesn't need location dropdown
            label = try_xp(Question, ".//label[@for]", False)
            label_org = label.text if label else "Unknown"
            label = label_org.lower()
            answer = ""
        ...
        try:
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    print_lg(f'Applied & recorded "{title} | {company}" (Job ID: {job_id})')
                    current_count += 1
